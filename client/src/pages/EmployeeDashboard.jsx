            ...
            <div className={`attendance-card ${isCheckedIn ? 'active-session' : ''}`}>
                <div className="att-info">
                    ...
                    <div className="session-status">
                        {isCheckedIn ? (
                            <span className="status-live">● Live: {calculateDuration(startTime).h}h {calculateDuration(startTime).m}m</span>
                        ) : (
                ...
                <div className="att-controls">
                    {isCheckedIn ? (
                        <>
                            ...
                            />
                            <button className="btn-action btn-out" onClick={handleCheckOut} disabled={loading}>
                                {loading ? '...' : <><StopCircle size={18}/> Check Out</>}
                            </button>
                        </>
                    ) : (
                        <button className="btn-action btn-in" onClick={handleCheckIn} disabled={loading}>
                             {loading ? '...' : <><PlayCircle size={18}/> Check In</>}
                        </button>
                    )}
                </div>
            </div>
        ...
        <div className="content-grid">
            ...
            <div className="task-section">
                <div className="section-header">
                    <h3><ListTodo className="icon-orange" size={22}/> My Tasks ({totalTasks})</h3>
                    {isContentWriter && (
                        <button className="btn-small-primary" onClick={() => setIsAssignModalOpen(true)}>
                        ...
                        tasks.map(task => (
                            <div key={task._id} className="task-card" onClick={() => {setSelectedTask(task); setIsViewModalOpen(true);}}>
                                ...
                                <div className="task-footer">
                                    ...
                                    <button 
                                        className={`btn-check ${task.status === 'Completed' ? 'checked' : ''}`}
                                        onClick={(e) => { e.stopPropagation(); handleToggleTask(task._id, task.status); }}
                                    >
                                        {task.status === 'Completed' ? <CheckCircle2 size={20}/> : <div className="circle-empty"></div>}
                                    </button>
                                </div>
                            </div>
                        ))
            ...
            <div className="sidebar-section">
                ...
                <div className="widget-card">
                    ...
                    <div className="info-row">
                        ...
                        <strong>{lastSession.checkInTime ? new Date(lastSession.checkInTime).toLocaleTimeString() : 'N/A'}</strong>
                    </div>
                    <div className="info-row">
                        <span>Pending Tasks</span>
                        <strong style={{color: '#ff7f50'}}>{totalTasks - completedTasks}</strong>
                    </div>
                </div>
            ...
            <div className="modal-window" onClick={e => e.stopPropagation()}>
                ...
                <div className="modal-body">
                    ...
                    <div className="modal-actions">
                        <button 
                            className={`btn-primary-fill ${selectedTask.status === 'Completed' ? 'green' : ''}`}
                ...
                <form onSubmit={handleAssignTask}>
                    <div className="modal-body">
                        <div className="input-group">
                            ...
                            <input type="text" required value={newTaskData.title} onChange={e => setNewTaskData({...newTaskData, title: e.target.value})} placeholder="e.g. Edit Vlog #4"/>
                        </div>
                        <div className="input-group">
                            <label>Description</label>
                            <textarea rows="3" required value={newTaskData.description} onChange={e => setNewTaskData({...newTaskData, description: e.target.value})} placeholder="Details..."/>
                        </div>
                        <div className="row-inputs">
                            <div className="input-group">
                                ...
                                >
                                    ...
                                    {videoEditors.map(v => <option key={v._id} value={v._id}>{v.name}</option>)}
                                </select>
                            </div>
                            <div className="input-group">
                        ...
                        <div className="input-group">
                            ...
                            <input type="date" required value={newTaskData.dueDate} onChange={e => setNewTaskData({...newTaskData, dueDate: e.target.value})}/>
                        </div>
                        <button type="submit" className="btn-primary-fill" style={{width:'100%', marginTop:'15px'}}>Assign Task</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div className="modal-backdrop" onClick={() => setIsTeammateModalOpen(false)}>
            <div className="modal-window wide" onClick={e => e.stopPropagation()}>
                <div className="modal-top">
                    ...
                    <button onClick={() => setIsTeammateModalOpen(false)}><X size={20}/></button>
                </div>
                        ...
                        <div className="teammate-sections">
                                        ...
                                        {selectedTeammateData.attendanceHistory.map((record, index) => (
                                            <div key={index} style={{ background: '#f9f9f9', padding: '15px', borderRadius: '10px', marginBottom: '12px', border: '1px solid #eee' }}>
                                                ...
                                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontWeight: '600', fontSize: '0.9rem' }}>
                                                    ...
                                                    <span style={{ color: record.checkOutTime ? '#2e7d32' : '#ed6c02', fontSize: '0.8rem', background: record.checkOutTime ? '#e8f5e9' : '#fff3e0', padding: '2px 8px', borderRadius: '4px' }}>
                                                ...
                                                {record.taskDescription ? (
                                                    ...
                                                    <div style={{ fontSize: '0.85rem', color: '#999', fontStyle: 'italic' }}>No description provided.</div>
                                                )}
                                            </div>
                                        ))}
                                    </div>
                            ...
                            <div className="section-block" style={{ marginTop: '30px' }}>
                                ...
                                {selectedTeammateData.tasks.length === 0 ? (
                                    ...
                                    <div className="mini-task-list" style={{ maxHeight: '250px', overflowY: 'auto', paddingRight: '5px' }}>
                                        {selectedTeammateData.tasks.map(t => (
                                            <div key={t._id} style={{ display: 'flex', justifyContent: 'space-between', padding: '12px', background: 'white', border: '1px solid #eee', borderRadius: '8px', marginBottom: '8px', alignItems: 'center' }}>
                                                <div style={{flex: 1}}>
                                                    <p style={{ fontWeight: '600', margin: '0 0 4px', fontSize: '0.95rem' }}>{t.title}</p>
