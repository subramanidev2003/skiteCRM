...
const InvoicePayment = () => {
  ...
  const handleUpdatePayment = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/invoice/update-payment/${selectedInvoice._id}`, {
        ...
        body: JSON.stringify({ paidAmount: Number(paymentAmount) + currentPaid })
      });
      const data = await res.json();
      if (res.ok) {
  ...
  const filteredInvoices = invoices.filter(inv => {
    ...
    let matchesGST = true;
    if (filterType === 'With GST') matchesGST = parseFloat(inv.taxRate) > 0;
    else if (filterType === 'Without GST') matchesGST = !inv.taxRate || parseFloat(inv.taxRate) === 0;
    ...
    if (fromDate || toDate) {
      ...
      if (toDate) {
        const tDate = new Date(toDate); tDate.setHours(0, 0, 0, 0);
    ...
    <div className="payment-container">
      ...
      <div style={{ display: 'flex', gap: '15px', marginBottom: '20px', flexWrap: 'wrap' }}>
        ...
        <div className="search-bar-wrapper" style={{ flex: 1, minWidth: '250px', display: 'flex', alignItems: 'center', background: 'white', padding: '10px 15px', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
          ...
          <input type="text" placeholder="Search by Client or Invoice No..." value={searchTerm}
        ...
        <table className="payment-table" style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          ...
          <tbody>
            {filteredInvoices.length === 0 ? (
              <tr><td colSpan="8" style={{ textAlign: 'center', padding: '30px', color: '#888' }}>No invoices found.</td></tr>
            ) : (
              filteredInvoices.map((inv) => {
                ...
                else if (paid > 0) { status = "Partial"; statusClass = "status-partial"; }
                ...
                return (
                  <tr key={inv._id} style={{ borderBottom: '1px solid #eee' }}>
                    ...
                    <td style={{ padding: '15px', textAlign: 'center' }}>
                      {balance > 0 ? (
                        <button className="pay-btn" onClick={() => openPaymentModal(inv)}
